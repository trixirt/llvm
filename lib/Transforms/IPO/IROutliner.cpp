...
private:
  ...
  unsigned computeOutlinedFunctionRegisterCost(Candidate &C) {
    ...
    unsigned i, InstIdx, LiveIntervals, Spill;

    /* Add the first nonfree insn to open interval */
    for (i = 0, InstIdx = FirstOccur; i < C.Len; ++i, ++InstIdx) {
      InstructionInfo &II = OM.getInstrInfo(InstIdx);
    ...
    for (; i < C.Len; ++i, ++InstIdx) {
      // Remove all of the instructions that end at this location.
      InstrList &List = TransposeEnds[i];
      if (List.size()) {
        /* How many registers were used before able to return any */
        LiveIntervals = 0;
        for (auto Inst : OpenIntervals)
          LiveIntervals +=
              getRegisterUsage(*Layout, Inst->getType(), WidestRegister);

        /* How many registers are being returned */
        unsigned int DeadIntervals = 0;
        for (unsigned ToRemove : List) {
          auto Inst = OM.getInstr(ToRemove);
          DeadIntervals +=
              getRegisterUsage(*Layout, Inst->getType(), WidestRegister);
          OpenIntervals.erase(Inst);
        }
        if (LiveIntervals > NumRegisters) {
          Spill = LiveIntervals - NumRegisters;
          /* Report spills on only what we can return */
          if (DeadIntervals < Spill) {
            Spill = DeadIntervals;
            LLVM_DEBUG(dbgs() << "\tShortSpill " << Spill << "\n");
          } else {
            LLVM_DEBUG(dbgs() << "\tSpill " << Spill << "\n");
          }
          SpillCount += Spill;
        }
      }
    ...
    /* Repeat the spill count on the trailing part */
    LiveIntervals = 0;
    for (auto Inst : OpenIntervals)
      LiveIntervals +=
          getRegisterUsage(*Layout, Inst->getType(), WidestRegister);
    if (LiveIntervals > NumRegisters) {
      Spill = LiveIntervals - NumRegisters;
      LLVM_DEBUG(dbgs() << "\tFinalSpill " << Spill << "\n");
      SpillCount += Spill;
    }
    LLVM_DEBUG(dbgs() << "\tTotal Spillcount " << SpillCount << "\n");
    return SpillCount;
  }
